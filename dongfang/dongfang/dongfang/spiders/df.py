...
#class DfSpider(scrapy.Spider): #将原来的父类写成继承为分布式的父类RedisSpider如下
class DfSpider(RedisSpider):
    name = 'df'
    allowed_domains = ['http://www.ocj.com.cn']
    #start_urls = ['http://www.ocj.com.cn']
    # 注释掉start_url,设置成分布式的
    redis_key='df:start_urls'
    # 分布式项目的起始点url为http://www.ocj.com.cn/
    # 在redis客户端redis-cli.exe下执行：lpush df:start_urls http://www.ocj.com.cn把起始url压入第一个url队列便可
    # 下面的爬取代码分布式和原来是一样的
   ...
   """
    ...
    # 2.观察首页只要得到div的class为hIndexLeftNavInfoListContentBoxSmallButton的节点下的a标签的href属性值
         ...
         catergory_list=response.xpath('//div[@class="hIndexLeftNavInfoListContentBoxSmallButton"]/a/@href').extract()
         ...
         for url in catergory_list:
             ...
             yield scrapy.Request(url=url,headers=self.headers,callback=self.parse_category,dont_filter=True)#将链接提交给本类中的parse_category函数进行处理,设置dont_filter= True是为了防止爬取到的url与allowed_domains出现问题而无法发起请求
             # 以上完成了分类商品列表链接的获取
         ...
         good_info_list=response.xpath('//div[@class="hItemShowImg"]/a/@href').extract()
         # (2)判断连接是否缺损，然后进行修正，并提交给另一个函数进行处理
         print(good_info_list)
         for url in good_info_list:
             if "http://www.ocj.com.cn" not in url:
                 url="http://www.ocj.com.cn"+url
             yield scrapy.Request(url=url,callback=self.parse_good_info,headers=self.headers,dont_filter=True)
             # print('***********************************' + url)
             # print(url)
             # time.sleep(2)
    ...
    def parse_category(self,response):
        ...
        # 这条连接是列表形式的链接
        good_info_url_list=response.xpath('//p[@class="title"]/a/@href').extract()
        ...
        Nextpage_url = response.xpath('//a[@class="page-next"]/@href').extract()
        # 对连接进行修正
    ...
    def parse_good_info(self, response):
        try:
            ...
            title = response.xpath('//div[@class="pv_shop_detail_title"]/h1/text()').extract()
            # 有一些商品的title的xpath路径不一样为response.xpath(// *[ @ id = "containerContent"] / div[1] / div[1] / h1 / text()).extract()
            # 故在这里对title做一个判断是否为空值，如果为空值这利用第二种的xpath方法爬取
            if title == None:
                title = response.xpath('//*[@id = "containerContent"]/div[1]/div[1]/h1/text()').extract()
            else:
                pass
            # //*[@id="containerContent"]/div[1]/div[2]/div[2]/div/dl[2]/dd/span
            # // *[ @ id = "containerContent"] / div[1] / div[2] / div[2] / div / dl[2] / dd / span
            price = response.xpath('//*[@id="containerContent"]/div[1]/div[2]/div[2]/div/dl[2]/dd/span/text()').extract()  # 价格转为数组后去掉价格中的逗号
            # id = response.xpath('//div[@class="info_box"]/div[@class="@no clear_float"]/div[@class="val"]/text()').extract()
            id = response.xpath('//div[@class="info_box"]/dl[1]/dd[1]/text()').extract()
            score = response.xpath('string(//p[@class="score"][1])').extract()  # string()可以获取当前标签的所以文本信息
            place = response.xpath('//div[@class="parameter"]/div[3]/text()').extract()
            comment = response.xpath('//*[@id="detail_bl_tt"]/ul/li[3]/span/label/em/text()').extract()
            # print(id[0])
            # print(title[0].strip())
